...
- **Node 8 LTS:** `serverless-test-utils` has a minimum Node version
...
``` bash
git clone https://github.com/EndemolShineGroup/serverless-test-utils.git
cd serverless-test-utils
yarn
yarn test
```
...
We want to ensure that `serverless-test-utils` is secure for everyone. If
